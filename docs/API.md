...
所有接口 HTTP 状态码大部分为 `200`（业务状态通过 `code` 字段区分），权限/验证错误时使用对应 HTTP 状态码：
...
|------|-------------|------|
| 200  | 200         | 成功 |
| 400  | 400         | 请求参数无效/模型绑定失败（自动改写为 `ApiResponse` 格式，不再返回默认 `ProblemDetails`） |
| 401  | 401         | 未认证 |
| 403  | 403         | 无权限 |
| 500  | 200 或 500  | 业务失败/服务端异常 |

- 具体接口在发生业务异常时，会返回 `code: 500` 和具体 `message`，`data` 为 `null`。
- 例如：角色编码重复、文献不存在、文献被借阅中无法删除、借阅额度已满等错误。
- 发生 EF Core `DbUpdateException`（如唯一键冲突、外键约束失败）时，系统自动穿透到内层真实错误信息返回，不再显示 "An error occurred while updating the entries" 这类通用信息。
- 所有未捕获异常由全局异常中间件统一捕获，始终返回 `ApiResponse` 格式 JSON，前端只需读取 `code` 与 `message` 字段即可。
...
业务逻辑在数据库事务中执行：校验用户正常、单人额度熔断（`当前流转中+逾期未还数 + 本次拟借数 > 5` 时拒绝）、禁止重复借阅（同一用户对同一文献有流转中或逾期记录时拒绝）、逐本校验文献存在且 `status = 1`、`stock > 0`，扣减库存并批量写入 `SysBorrowLog`。任一文献不满足条件时整批回滚。
...
1. 单人额度熔断：`当前流转中(LogStatus=0) + 逾期未还(LogStatus=2) 的总数 + 本次拟借数 > 5` → `"您的借阅额度已满（单人上限 5 本），请先归还现有文献。"`
2. 禁止重复借阅：该用户对同一文献存在流转中或逾期未还记录 → `"您已借阅过文献《XXX》，在归还前无需重复借阅。"`
